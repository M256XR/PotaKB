...
void setup() {
  ...
  usb_hid.setPollInterval(1);  // 1ms = 1000Hz
  ...
  initMCP23017();
  Wire.setClock(400000);  // I2C高速化: 100kHz → 400kHz（マトリクススキャンが4倍速）
...
void initBluetooth() { 
  ...
  Bluefruit.Periph.setConnInterval(6, 6);  // 7.5ms固定（最高速） 
...
bool processAllInputs() {
  ...
  auto process_a_key = [&](uint16_t key) {
    switch (key) {
      ...
      case 0x501:  // KC_RESET_ALL - キーマップと設定を両方リセット
      {
        // 紫色LED（赤+青）で5回点滅
        for(int i=0; i<5; i++){ setXiaoLed(255,0,255); delay(100); setXiaoLed(0,0,0); delay(100); }
        InternalFS.remove(KEYMAP_FILENAME);  // キーマップ削除
        InternalFS.remove(CONFIG_FILENAME);  // 設定削除
        delay(100);
        NVIC_SystemReset();  // 再起動してデフォルトに戻す
        break;
      }
      case 0x502:  // KC_REBOOT_DEF - デフォルトキーマップで起動
      {
...
if (active_layer == 1) {
      ...
      if (speed_magnitude > currentConfig.stick_deadzone) {
        float dir_mag = sqrt(xDiff * xDiff + yDiff * yDiff); 
        if (dir_mag < 1) dir_mag = 1; 
        float dirX = xDiff / dir_mag; 
        float dirY = yDiff / dir_mag; 
        
        // シンプルな線形加速（デッドゾーンから最大まで一定のペースで上がる）
        float progress = (speed_magnitude - currentConfig.stick_deadzone) / (511.0f - currentConfig.stick_deadzone);
        float speed = progress * currentConfig.mouse_high_speed;
  ...
  // マウスレポートを定期的に送信（スクロールと同じアプローチ）
  unsigned long currentTime = millis();
  bool should_send_mouse = false;
  
  // USB: 1ms間隔 (1000Hz), Bluetooth: 7ms間隔 (143Hz)
  uint16_t mouse_interval = (currentMode == MODE_USB) ? 1 : 7;
  
  if (currentTime - lastMouseReportTime >= mouse_interval) {
    lastMouseReportTime = currentTime;
  ...
  // 定期送信は必ず行う（値が0でも送信して1000Hz/143Hzを保証）
  ...
  // または重要な変化があれば追加で送信
  else if (mouseX_to_send != 0 || mouseY_to_send != 0 || 
      mouse_buttons != prev_mouse_buttons || 
...
void scanMatrix() { 
  static int currentCol = 0;  // スキャンする列を分割
  unsigned long currentTime = millis(); 
  ...
  currentCol = (currentCol + 1) % COLS;  // 次の列へ
}
