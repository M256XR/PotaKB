...
// スクロール関連の定数を追加
#define SCROLL_MIN_THRESHOLD 30   // スクロールを開始する最小の傾き
#define SCROLL_MAX_THRESHOLD 300  // 最大スクロール速度に達する傾き
#define SCROLL_MIN_SPEED 1        // 最小スクロール量
#define SCROLL_MAX_SPEED 5        // 最大スクロール量
#define SCROLL_INTERVAL_MS 50    // スクロール更新間隔(ミリ秒)
...
void updateBatteryLevel() {
  ...
  if (volt <= 3.5) {
    ...
    battery_percent = (uint8_t)(((volt - 3.62) * 277.78 + 5) / 5 + 0.5) * 5;
  } else if (volt <= 4.1) {
    battery_percent = (uint8_t)(((volt - 3.8) * 150 + 55) / 5 + 0.5) * 5;
  } else {
    battery_percent = 100;
  }
...
bool processAllInputs() {
  ...
  if (millis() - lastMoveTime >= 1) {
    ...
    if (active_layer == 1) {
      // スクロールモード: 傾き具合によってスクロール量を変化
      unsigned long currentTime = millis();
      
      // Y軸スクロール(上下)
      if (abs(yDiff) > SCROLL_MIN_THRESHOLD) {
        float magnitude = abs(yDiff);
        float scroll_speed;
        
        // 傾き具合に応じてスクロール速度を計算
        if (magnitude > SCROLL_MAX_THRESHOLD) {
          scroll_speed = SCROLL_MAX_SPEED;
        } else {
          // 線形補間で速度を計算
          scroll_speed = SCROLL_MIN_SPEED + 
                        (magnitude - SCROLL_MIN_THRESHOLD) / 
                        (SCROLL_MAX_THRESHOLD - SCROLL_MIN_THRESHOLD) * 
                        (SCROLL_MAX_SPEED - SCROLL_MIN_SPEED);
        }
      ...
      // X軸スクロール(水平)
      if (abs(xDiff) > SCROLL_MIN_THRESHOLD) {
        float magnitude = abs(xDiff);
        float pan_speed;
        
        if (magnitude > SCROLL_MAX_THRESHOLD) {
          pan_speed = SCROLL_MAX_SPEED;
        } else {
          pan_speed = SCROLL_MIN_SPEED + 
                     (magnitude - SCROLL_MIN_THRESHOLD) / 
                     (SCROLL_MAX_THRESHOLD - SCROLL_MIN_THRESHOLD) * 
                     (SCROLL_MAX_SPEED - SCROLL_MIN_SPEED);
        }
      ...
      if (currentTime - lastScrollTime >= SCROLL_INTERVAL_MS) {
        ...
        // アキュムレータから整数部分を取り出して送信
        if (abs(scroll_accumulator) >= 1.0) {
          mouse_scroll = (int8_t)scroll_accumulator;
          scroll_accumulator -= mouse_scroll;
        }
        
        if (abs(pan_accumulator) >= 1.0) {
          mouse_pan = (int8_t)pan_accumulator;
          pan_accumulator -= mouse_pan;
        }
      }
...
void switchMode(OperatingMode_t newMode) { currentMode = newMode; if (newMode == MODE_USB) { if (Bluefruit.connected()) { Bluefruit.disconnect(Bluefruit.connHandle()); delay(100); } Bluefruit.Advertising.stop(); setXiaoLed(0, LED_BRIGHTNESS, 0); analogWrite(BLUETOOTH_LED_PIN, 0);} else { Bluefruit.Advertising.start(0); setXiaoLed(0, 0, LED_BRIGHTNESS); } lastActivityTime = millis(); }
